...
const CountdownTimer = ({ eventId, remainingTime, loading }: CountDownTimerProps) => {
        ...
        {(showClosed || showTimer || showNotOpenYet) && (
          <Stack direction="row" gap={2} alignItems="center">
            <Avatar
              sx={{
                bgcolor:
                  remainingTime === 'Not Open Yet' ? 'success.lighter' : remainingTime === 'Closed' ? 'error.lighter' : 'primary.lighter',
              }}
              variant="rounded"
              component="div"
            >
              <Clock size="30" color={remainingTime === 'Not Open Yet' ? '#00A854' : remainingTime === 'Closed' ? '#F04134' : '#3C64D0'} />
            </Avatar>
          </Stack>
        )}
