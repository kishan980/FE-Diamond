import React, { useEffect, useState, Dispatch, Fragment, MouseEvent, SetStateAction, useMemo } from 'react';
...
import { NavGroupStyledPaper } from './Navigation.styled';
...
const NavGroup = ({ item, lastItem, remItems, lastItemId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }: Props) => {
  ...
  const maximumPurchaseLimit = searchParams.get('maximumPurchaseLimit');
  const showOverAllPurchaseLimit = searchParams.get('showOverAllPurchaseLimit');
  ...
  const isSelected = useMemo(() => {
    ...
    return selectedID === currentItem.id;
  }, [pathname, selectedID, currentItem.id]);
  ...
  const handleItemClick = (event: MouseEvent<HTMLDivElement>) => {
    ...
    if (showOverAllPurchaseLimit === 'Yes' && Number(maximumPurchaseLimit) === 0) {
      toast.error('Please first define your Purchase Limit in order to be able to further proceed and to submit your bids.');
      return;
    }

    if (currentItem.url) router.push(currentItem.url);
    handleClick(event);
  };
  return downLG ? (
    <List
    ...
    >
      {navCollapse ? navCollapse : <NavItem item={item} level={1} />}
    </List>
  ) : (
    <List sx={{ py: 0 }}>
      <ListItemButton
        selected={isSelected}
        sx={{ p: 0.25, px: 1, my: 0, mr: 1, display: 'flex', alignItems: 'center', borderRadius: 1 }}
