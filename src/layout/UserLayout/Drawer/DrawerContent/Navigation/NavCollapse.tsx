...
import { NavInnerStyledPaper, NavStyledPaper } from './Navigation.styled';
  ...
  useMemo(() => {
    if (selected === selectedItems) {
      ...
      if (level === selectedLevel) {
        setOpen(false);
        if ((!miniMenuOpened && !drawerOpen && !selected) || drawerOpen) setSelected(null);
      }
    }
  ...
  return downLG ? (
    <>
      <ListItemButton
        ...
        sx={{
          ...
          py: !drawerOpen && level === 1 ? 0.5 : 0.25,
          ...(drawerOpen && {
            mx: 1.25,
            my: 0,
            borderRadius: 1,
    ...
    >
      <Box onClick={handlerIconLink} sx={FlexBox}>
        {menuIcon && (
          <ListItemIcon sx={{ my: 'auto', minWidth: !menu.icon ? 18 : 36, color: theme.palette.secondary.dark }}>{menuIcon}</ListItemIcon>
        )}
        <ListItemText
          primary={
