...
import { NavGroupStyledPaper } from './NavCollapse.styled';
...
const NavGroup = ({ item, lastItem, remItems, lastItemId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }: Props) => {
  ...
  const textColor = theme.palette.mode === ThemeMode.DARK ? 'secondary.400' : theme.palette.secondary.darker;
  ...
  return downLG ? (
    ...
    <List sx={{ py: 0 }}>
      <ListItemButton
        ...
        {...(currentItem.url && { href: currentItem.url, component: Link })}
      >
        {itemIcon && (
